...
class Restic(nagiosplugin.Resource):
    ...
    def __init__(self, restic_bin='restic', host=None, path=None, repo=None, password_file=None, sudo=False):
    ...
    def probe(self):
        ...
        try:
            ...
            raise nagiosplugin.CheckError('Failed to run %s: %s' % (' '.join(cmd), e.output.decode()))
        except FileNotFoundError as e:
            raise nagiosplugin.CheckError('Failed to run %s: %s' % (' '.join(cmd), e))
        _log.debug('Got output: %s' % restic_result)

        try:
            snapshots = json.loads(restic_result)
        except json.decoder.JSONDecodeError as e:
            raise nagiosplugin.CheckError('Unable to parse restic output: %s' % e)
        ...
        while True:
            try:
                e = next(e for e in snapshots if '_'.join(e['paths']) not in last_snapshots.keys())
                last_snapshots['_'.join(e['paths'])] = e
            except StopIteration:
                break

        for path, snapshot in last_snapshots.items():
            snapshot_age = datetime.datetime.now(None) - dateutil.parser.parse(snapshot['time']).replace(tzinfo=None)
            snapshot_age = snapshot_age.total_seconds() / (60*60)
            yield nagiosplugin.Metric(path, snapshot_age, uom='h', context='last_snapshot_age')
...
class ResticSummary(nagiosplugin.Summary):
    def ok(self, results):
        ...
        :return:
        """
        ret = ['%s is %.2f hours old' % (r.metric.name, r.metric.value) for r in results]
    ...
    def problem(self, results):
        ...
        ret = ['%s is %2f hours old' % (r.metric.name, r.metric.value) for r in results if r.state != nagiosplugin.Ok]
        return 'Snapshot %s' % ', '.join(ret)
...
def main():
    ...
    argp.add_argument('--sudo', action='store_true',
                      help='Use "sudo" when invoking restic (default: %(default)s)')
    argp.add_argument('--restic-bin', type=str, metavar='RESTIC-BIN', default='restic',
                      help='Path to the restic binary, or the name of restic in $PATH (default: %(default)s)')
    ...
    check = nagiosplugin.Check(
        Restic(restic_bin=args.restic_bin, host=args.host, path=args.path, repo=args.repo,
               password_file=args.password_file, sudo=args.sudo),
